...
nwis_filtered<-nwis_data %>% 
  ...
  select(date=Date,SiteID, year, SpecCond,month)
  

year_stats<-nwis_filtered %>% 
  group_by(SiteID, year) %>% 
  summarise(mean=mean(SpecCond, na.rm = TRUE),
            n=n())%>%
  nest()
...
joined_data<-inner_join(nwis_prep,conc.daily, by=c("SiteID", "date"))
...
ggplot(joined_data)+
  geom_point(aes(x=SpecCond, y=conc, color=SiteID))+
...
joined_nest<-joined_data %>% 
  group_by(parameter) %>% 
  nest()


lm_map <- function(df){
  model <- lm(formula= SpecCond~conc,
              data = joined_data ) %>% 
    tidy()
}

lm_results <- joined_nest%>%
...
#### All of the ions are correlated heavily with Specific Conductivity because they all make up Specific Conductivity as a whole. 
...
q_data <- readNWISdv(siteNumbers = siteNumbers,
  ...
  mutate(year=year(.$Date),
    month=month(.$Date),
    Q_ls=Flow*28.315) %>% 
  select(date=Date,SiteID, Q_ls)

q_joined<-inner_join(joined_data,q_data, by=c("SiteID","date"))
...
lm_map <- function(df){
  model <- lm(formula= SpecCond~conc*Q_ls,
              data = q_joined )%>%
    tidy()
}
