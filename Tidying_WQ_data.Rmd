...
# remove the USGS part of the site code for read function, how I originally did this - 
#focus_sites <- focus_sites %>% 
#  separate(sites, into = c("usgs", "site_num"), sep = "-") %>% 
#  select(-usgs)
# if i had does it this way, siteNumbers = focus_sites$sites in function

# this is a quicker way to remove USGS for function use
site_nos <- gsub("USGS-","", focus_sites$sites) 
# explaination: gsub("look for pattern", "replace with this", "look in this df$column")
...
sc_data <- readNWISdv(
  ...
  endDate = "2024-03-20") %>% 
  renameNWISColumns() # a function included in this package that automatically renames their long column headings
# find more info on this package - https://cran.r-project.org/web/packages/dataRetrieval/dataRetrieval.pdf

sc_data %>% 
  ggplot(mapping = aes(x = Date, y = SpecCond, color = site_no)) +
  geom_point() +
  labs(title = "Specific Conductivity, 2007 - present",
      x = "date", 
      y = "ms/cm")
```
...
sc_data_summer <- sc_data %>% 
  filter(month(Date) %in% c(7, 8, 9, 10)) %>% # use month function in lubridate to filter by month
  mutate(site_no = paste0("USGS-", site_no))  # add USGS back to site_no bc of future joining

  
sc_data_sum <- sc_data_summer %>% 
  group_by(site_no, year(Date)) %>% 
  summarize(annual_mean = mean(SpecCond)) %>% # create new column called annual mean
  rename("year" = "year(Date)") %>% # rename year column since summarize will call it year(Date)
  nest()
...
# create function to loop through sens.slope function to generate stats for each site

sens <- function(df){
  model <- sens.slope(x = df$annual_mean) %>% 
    tidy()
} # make sure to run functions from the end curly bracket

spec_sens_results <- sc_data_sum %>% 
  mutate(models = map(data, sens)) %>% # data is the column name, sens is the function name
  unnest(cols = models) # this is saying to make the columns each of the stat model results

```

#### We can see that there is a trend at site USGS-09180000, but none of the other sites have a low enough p-value to indicate a trend.
...
ion_data <- left_join(conc.daily, sc_data, 
                      by = c("SiteID" = "site_no", "date" = "Date")) %>% 
...
```{r}
# I want to facet plots by ion, data colored by site

ion_data %>% 
  ggplot(mapping = aes(x = SpecCond, y = conc, color = SiteID)) +
  geom_point() +
  facet_wrap( ~ parameter, scales = "free") +
  labs(title = "Concentration of ions v specific conductivity", 
...
```{r}
# very similar to what we did with sens.slope, so I copied code down and revised
# i need to nest the tibble, like above

conc_sc <- ion_data %>% 
  group_by(parameter) %>% 
  nest()

conc_sc_stat <- function(df){
  model <- lm(conc ~ SpecCond, data = df) %>% 
    tidy()
}

lm_results <- conc_sc %>% 
  mutate(models = map(data, conc_sc_stat)) %>% # data is the column name, conc_sc_stat is the function name
  unnest(cols = models) # this is saying to make the columns each of the stat model results
...
#### Unsurprisingly, all the measured ions are correlated with SC since SC is a measure of dissolved ions in solution! We can see this because all p-values are very small; the smallest p-value being that for Sodium at 1.4\*10\^-280, that is a tiny value.
...
```{r}
# download discharge data for sites

disch_code <- '00060'

# read in discharge data, like specific condiuctivity data in Q1
disch_data <- readNWISdv(
  siteNumbers = site_nos,
  parameterCd = disch_code,
  startDate = "2007-01-01",
  endDate = "2024-03-20") %>% 
  renameNWISColumns()

# adding usgs SiteID col to join with ion data, rename date column to match as well
disch_data <- disch_data %>% 
  mutate(SiteID = paste0("USGS-", site_no)) %>% 
  rename(date = Date) %>% 
  select(-agency_cd, -site_no)

# join discharge data with ion and sc data, use left join with ion table as primary to avoid NAs
disch_ion_data <- left_join(ion_data, disch_data,
                            by = c("SiteID", "date")) %>% 
  filter(!is.na(Flow))

# plot discharge v SC
disch_ion_data %>% 
  ggplot(mapping = aes(x = Flow, y = SpecCond, color = SiteID)) +
  geom_point() +
  labs(title = "Discharge v Specific Conductivity",
       subtitle = "(limited to lower values)",
       x = "Discharge",
       y = "Specific Conductivity") +
  ylim(0, 4000) +
  xlim(0, 5000)

# plot discharge v ions to show that it's very similar to discharge v SC
disch_ion_data %>% 
  ggplot(mapping = aes(x = Flow, y = conc, color = SiteID)) +
  geom_point() +
  facet_wrap( ~ parameter, scales = "free") +
  labs(title = "Discharge v Ion Concentration",
       x = "Discharge",
       y = "Concentration")

```

#### From plotting discharge vs specific conductivity, it looks like specific conductivity decreases with increasing discharge. This can especially be seen by coloring data points by site. In answer to the question about does this alter the relationship between SC and ions.. when plotting discharge against ion concentration, we can see that a similar relationship to SC exists, where ion concentation decreases with increasing discharges. This makes sense after seeing the linear relationship between SC and ion concentrations. I don't think plotting discharges against ions really gives us any more information than plotting discharge against SC unless we were curious about a specific ion. 
