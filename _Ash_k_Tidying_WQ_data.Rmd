...
```{r}
# Nesting the data by parameter and SiteID
sc_ions_nested <- sc_ions %>%
  group_by(parameter, SiteID) %>%
  nest()

# Defining a function for linear modeling - concentration as a function of specific conductivity
model_sc_ions <- function(df) {
  model <- lm(conc ~ SpecCond, data = df)
  return(model)
}

# Applying the model function to the nested data
sc_ions_model_results <- sc_ions_nested %>%
  mutate(models = map(data, model_sc_ions))

# Extracting model summaries and relevant information
sc_ions_correlations <- sc_ions_model_results %>%
  mutate(summary = map(models, summary),
         adj_r_squared = map_dbl(summary, ~ .x$adj.r.squared),
         p_value = map_dbl(summary, ~ .x$coefficients["SpecCond", "Pr(>|t|)"]))

# Selecting and arranging the results
sc_ions_correlations %>%
  select(parameter, SiteID, adj_r_squared, p_value) %>%
...
All of the ions are correlated with SC. However ions with largest colleration is Sulfate, Sodium, Chloride and Potassium. Specific conductivity is solution (inour case water's) abilitity to conduct electricity.  The ions mentioned (especially Na and Cl and SO4) are commonly associated with salinity and soluable salts in water. so this high correlation with these ions suggest that they are major contributoers to the rivers in Colorado, most likey due to the weathering of rocks and anthropogenic activities (agriculture runfodd/ industrial discharge)


## Q3) How does discharge control these relationships?
##flow data to pull (00060 Q paramater)
...
```{r}
# Discharge parameter code
discharge_param = '00060'

# Download discharge data, focusing on July-October as before
discharge_info <- readNWISdv(siteNumbers = site_nos,
...
# Creating a combined dataframe with discharge and specific conductivity data
discharge_cond <- discharge_info %>%
  mutate(SiteCode = paste0('USGS-', site_no)) %>%
  inner_join(sc, by = c("SiteCode" = "SiteID", "analysis_date" = "date"))
...
```{r}
# Ensure 'focus_sites' dataframe includes a 'basin' column that can be joined with 'discharge_ion_cond'
# Join 'focus_sites' with 'discharge_ion_cond' to include the 'basin' information
discharge_ion_cond <- discharge_ion_cond %>%
  inner_join(focus_sites, by = c("SiteCode" = "sites"))

# Now, 'basin' should be part of 'discharge_ion_cond', and you can visualize the relationships as intended
ggplot(data = discharge_ion_cond) +
  geom_point(aes(x = Flow, y = SpecCond, color = basin)) +
  facet_wrap(~parameter, scales = "free") +
  labs(x = "Discharge (cfs)", y = "Specific Conductivity (µS/cm)") +
  theme_minimal() +
  ggtitle("Discharge vs. Specific Conductivity by Ion")


```

```{r}
# Function to model the relationship between discharge and specific conductivity
model_flow_cond <- function(df) {
  lm_result <- lm(Flow ~ SpecCond, data = df)
  return(summary(lm_result))
}

# Applying the model to the combined dataset
flow_cond_results <- discharge_ion_cond %>%
  group_by(parameter, SiteCode) %>%
  nest() %>%
  mutate(model_results = map(data, model_flow_cond))

# Extracting and summarizing model results
model_summaries <- flow_cond_results %>%
  unnest(model_results) %>%
  select(parameter, SiteCode, r.squared, p.value) %>%
  arrange(desc(r.squared))

# Displaying model summaries in a table
kable(model_summaries, "html") %>%
  kable_styling() %>%
  scroll_box(width = '600px', height = '500px')
...
# Adjusted function to model the relationship and return a tidied summary
model_flow_cond <- function(df) {
  lm_result <- lm(Flow ~ SpecCond, data = df)
  # Use glance from broom to get a one-row summary
  return(glance(lm_result))
}
...
flow_cond_results <- discharge_ion_cond %>%
  ...
  nest() %>%
  mutate(model_results = map(data, model_flow_cond)) %>%
  unnest(model_results)  # Unnesting the tidied model results
...
I wonder for Bicarbonate at 	USGS-09180500 with r-squared value of 1 but with p-value of NAN might be an error. 
However other than that overall,  for sites and ions where the relationship is strong and significant (high R-squared and low p-value), specific conductivity could be a reliable predictor of discharge, possibly due to consistent ion concentrations contributing to conductivity and affected by flow rates in the summer months.The concentration of ions in streams here can vary significantly from upstream (higher stream) areas to downstream (lower stream, dolores) areas, often resulting in higher ion concentrations in the lower reaches of a stream or river.
